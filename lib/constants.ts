...
export const CONTACT = {
   ...
   *
   * ⚠️ speaky.co.kr 에는 수신용 MX 레코드가 없다. 발송(Resend/SES)만 설정돼 있어
   *    @speaky.co.kr 로 온 메일은 받을 수 없다. 문의 창구는 실제로 수신되는
   *    도메인이어야 한다 — onthegostudio.kr 은 Zoho 메일이 붙어 있다.
   *
   *    이전 값은 'speaky@support.com' 이었는데 support.com 은 우리 도메인이 아니다.
   *    개인정보처리방침이 고지한 "권리 행사 창구" 가 남의 회사로 가고 있었다.
   */
  SUPPORT_EMAIL: 'hello@onthegostudio.kr',
} as const;

/**
 * 사업자 정보 (전자상거래법 제10조 표시 의무).
 *
 * 통신판매업자는 상호·대표자·주소·전화번호·이메일·사업자등록번호·통신판매업
 * 신고번호를 이용자가 쉽게 알 수 있도록 표시해야 한다. 결제대행(PG) 심사도 같은
 * 항목을 확인한다.
 *
 * ⚠️ 이 값들은 사업자등록증과 일치해야 한다. 바꿀 일이 생기면 등록증을 먼저 본다.
 */
export const BUSINESS = {
  ...
  CEO: '전진주',
  /** 사업자등록번호 */
  REGISTRATION_NO: '899-28-01873',
  /**
   * 통신판매업 신고번호.
   *
   * ⚠️ 아직 신고 전이라 비어 있다. 값이 없으면 화면에 줄 자체를 그리지 않는다 —
   *    "준비중" 같은 문구를 넣으면 표시 의무를 지킨 것으로 보이지도 않고,
   *    심사에서도 미표시와 똑같이 취급된다. 번호가 나오면 여기만 채우면 된다.
   */
  MAIL_ORDER_NO: '',
  /** 사업장 소재지 */
  ADDRESS: '충청남도 천안시 서북구 불당23로 70, 702-H01호(불당동)',
  /** 유선 전화번호 — PG 심사가 휴대폰이 아닌 유선번호를 요구한다 */
  PHONE: '041-414-6474',
  /** 고객 문의 이메일 */
